...
import { SFProject, SFWorkExperience, SFSkill, SFPortfolioSettings, SFCertification } from '@/types/salesforce';
...
export const sfConnection = globalForSF.sfConnection || new jsforce.Connection({
...
export async function loginToSalesforce() {
  ...
  if (!username || !clientId || !privateKey) {
    console.warn('Salesforce OAuth credentials not found in environment variables. Returning mock data or failing gracefully.');
    return false;
  }
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(`Failed to authenticate with JWT: ${data.error_description || data.error}`);
    }
...
export async function getCertifications(): Promise<SFCertification[]> {
  ...
  try {
    const result = await sfConnection.query(
      'SELECT Id, MasterLabel as Name, Issue_Date__c FROM Certification__mdt ORDER BY Order__c ASC NULLS LAST'
    );
    return result.records as unknown as SFCertification[];
  } catch (error) {
